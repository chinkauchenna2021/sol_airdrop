...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Get comprehensive user data including Twitter engagements
    const user = await prisma.user.findUnique({
      where: { id: userId },
      include: {
        _count: {
        ...
        engagements: true // Include TwitterEngagement data
      }
    })
    ...
    // UPDATED: Return separate points and tokens data with Twitter engagement
    const response = {
      user: {
        id: user.id,
        walletAddress: user.walletAddress,
        totalPoints: user.totalPoints,              // Points balance
        totalTokens: user.totalTokens,              // Token balance
        totalEarnedTokens: user.totalEarnedTokens,  // Lifetime tokens
        rank,
        twitterUsername: user.twitterUsername,
        twitterId: user.twitterId,
      ...
      referrals: {
        totalReferrals: referralData.length,
        activeReferrals: referralData.filter(r => r.completed && r.referred.isActive).length,
        totalEarnedTokens: totalEarned._sum.tokens || 0,
        recentReferrals: referralData.map(r => ({
          id: r.id,
          walletAddress: r.referred.walletAddress,
          twitterUsername: r.referred.twitterUsername,
