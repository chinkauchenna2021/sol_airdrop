 ...
 */
async function getSolAmountForUsd(usdAmount: number = 7) {
  try {
...
async function generatePaymentTransaction(userWallet: string) {
  try {
...
async function processPaidMint(userWallet: string, paymentSignature: string, userId: string) {
  try {
...
async function mintAndTransferNFT(recipientAddress: string, nftNumber: number) {
  try {
