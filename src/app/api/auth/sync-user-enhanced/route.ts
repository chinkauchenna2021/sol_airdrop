...
export async function POST(request: NextRequest) {
  try {
    const { walletAddress, betterAuthUser, twitterData } = await request.json();
    
    if (!walletAddress || !betterAuthUser) {
      return NextResponse.json(
        { error: "Wallet address and user data are required" },
        { status: 400 }
      );
    }
    ...
    if (!user) {
      ...
      user = await prisma.user.create({
        data: {
          walletAddress,
          twitterId: twitterData?.id || betterAuthUser.twitterId,
          twitterUsername: twitterData?.username || betterAuthUser.twitterUsername,
          twitterName: twitterData?.name || betterAuthUser.twitterName,
          twitterImage: twitterData?.image || betterAuthUser.twitterImage,
          twitterFollowers: twitterData?.followers_count || betterAuthUser.twitterFollowers,
          twitterActivity: "LOW",
          totalPoints: 0,
          totalTokens: 0,
          totalEarnedTokens: 0,
      ...
      // Award tokens for new user connection
      const awardedTokens = 100; // Example: 100 tokens for connecting Twitter
      await prisma.user.update({
        where: { id: user.id },
        data: {
          totalTokens: { increment: awardedTokens },
          totalEarnedTokens: { increment: awardedTokens }
        }
      });
      ...
      await prisma.pointHistory.create({
        data: {
          userId: user.id,
          tokens: awardedTokens,
          type: "TOKENS",
          action: "TWITTER_CONNECTION_BONUS",
          description: "Bonus for connecting Twitter account",
        }
      });
      
      return NextResponse.json({ 
        user, 
        awarded: awardedTokens,
        newUser: true 
      });
    } else {
      ...
      user = await prisma.user.update({
        where: { id: user.id },
        data: {
          twitterId: twitterData?.id || betterAuthUser.twitterId || user.twitterId,
          twitterUsername: twitterData?.username || betterAuthUser.twitterUsername || user.twitterUsername,
          twitterName: twitterData?.name || betterAuthUser.twitterName || user.twitterName,
          twitterImage: twitterData?.image || betterAuthUser.twitterImage || user.twitterImage,
          twitterFollowers: twitterData?.followers_count || betterAuthUser.twitterFollowers || user.twitterFollowers,
        }
      });
